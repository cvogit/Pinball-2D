...
public class ParticleMagnetController : MonoBehaviour {
	...
	private static ParticleSystem.Particle[] particleList;
	...
	void ParticlePull ()
	{
		...
		particleList = new ParticleSystem.Particle[gameObject.GetComponent<ParticleSystem>().particleCount + 1]; 

		int particleCount = gameObject.GetComponent<ParticleSystem>().GetParticles(particleList);

		for (int i = 0; i < particleCount; i++)
		{
			Vector3 offset = MagnetPoint.localPosition - particleList[i].position;
			...
			{
				particleList[i].position = Vector3.Lerp(particleList[i].position, MagnetPoint.localPosition, Mathf.SmoothStep(0, 2, (Time.deltaTime / 0.1F)));
				...
				if ((particleList[i].position-MagnetPoint.localPosition).magnitude <= 30)
				{
		...
		gameObject.GetComponent<ParticleSystem>().SetParticles(particleList, particleCount);
		return;
	}
